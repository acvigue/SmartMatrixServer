...
def bing_reverse_geo(coordinates, key):
    ...
    if data:
        ...
        if request.status_code != 200 or response.get("statusCode", False) != 200:
            ...
            cache_redis.set(cache_id, json.encode(address_parts), ttl_seconds = CACHE_TTL["location"])
...
def ors_directions(origin, destination, mode, key, **kwargs):
    ...
    if data:
        ...
        if request.status_code != 200:
            ...
            data = response
            cache_redis.set(cache_id, json.encode(response), ttl_seconds = CACHE_TTL["directions"])
...
def bing_directions(origin, destination, mode, key, **kwargs):
    ...
    if data:
        ...
        if request.status_code != 200 or response.get("statusCode", False) != 200:
            ...
            data = response
            cache_redis.set(cache_id, json.encode(response), ttl_seconds = CACHE_TTL["bing"])
