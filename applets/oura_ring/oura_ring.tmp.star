...
cache_redis.connect("**REDACTED**", "default", "**REDACTED**", 11389794)
load("http.star", "http")
...
def main(config):
    ...
    sleep_data = None
    sleep_dto = cache_redis.get("sleep_data")
    if sleep_dto != None:
        ...
        sleep_data = rep.json()
        cache_redis.set("sleep_data", json.encode(sleep_data), ttl_seconds = 1800)

    activity_data = None
    activity_dto = cache_redis.get("activity_data")
    if activity_dto != None:
        ...
        cache_redis.set("activity_data", json.encode(activity_data), ttl_seconds = 1800)

    readiness_data = None
    readiness_dto = cache_redis.get("readiness_data")
    if readiness_dto != None:
        ...
        cache_redis.set("readiness_data", json.encode(readiness_data), ttl_seconds = 1800)
